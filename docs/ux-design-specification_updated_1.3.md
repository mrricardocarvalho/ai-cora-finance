...
_Updated on 2025-11-28 (v1.3) - Final Polish: Privacy, Shortcuts, Pagination_
...
| **Transaction Row** | Swipeable (mobile) with quick categorization actions and confidence states |
...
| **IRS Badge** | Specialized indicator for Portuguese tax opportunities |
| **Safe-to-Spend Widget** | Real-time calculation of disposable liquidity vs. comfort floor |
| **Privacy Shield** | Visual indicator of encryption status and data control |
...
| **Tax/IRS** | Terracotta | `#E17055` | Portuguese tax insights and deadlines |
...
**Localization Rules (Strict):**
- **Currency:** Must follow Portuguese standard: `1.234,56 €` (Symbol at end, comma for decimals, dot for thousands).
...
- Health Score gauge
- Portuguese flag / IRS Shield for tax features
- Cora avatar/mascot
...
**Chat Interaction Physics:**
- **State Change:** Tapping the Chat Input does NOT just push the feed up.
- **Action:** It expands a **Bottom Sheet** (mobile) or **Side Panel** (desktop) that overlays the content.
- **Rationale:** Separates "Browsing Mode" (Feed) from "Interaction Mode" (Chat) to prevent context loss.
...
│  [Health Score]  │                                         │
│  [Safe-to-Spend] │                                         │
│  [Privacy Shield]│ <--- New Trust Indicator                │
│                  │                                         │
└──────────────────┴─────────────────────────────────────────┘
```
...
- **Safe-to-Spend** prominently displayed (Current Balance - Comfort Floor - Pending Bills)
...
- Statement upload area with **Privacy Indicators**
- Transaction review queue with **Bulk Edit** capabilities
...
- **Statement Upload:** Drag-drop with clear progress and **Privacy Shield** visible.
...
- **Gamified Feedback:** Actions explicitly show "+5 Health Score" toast.
...
#### Journey 4: Statement Upload & Review (with Bulk Edit)

**Goal:** Get transaction data into Cora with minimal effort and maximum trust.

**Approach:** Transparent AI Processing + Bulk Corrections.

```mermaid
flowchart TD
    A[Tap Upload Button] --> B[Select PDF/Drag-Drop]
    B --> C[Visual Processing Loop]
    C --> D{Parsing Successful?}
    D -->|Yes| E[Receipt Summary]
    D -->|No| F[Manual Fallback]
    E --> G{Review Low Confidence?}
    G -->|Yes| H[Select Multiple Items]
    H --> I[Bulk Edit Category]
    I --> J[Apply to All]
    J --> K[View Dashboard]
    G -->|No| K
```

**UX Details - Transparent Processing:**
- Instead of a generic spinner, show what the AI is doing.
- **Visual Loop:** "Detecting bank format (Moey)..." $\rightarrow$ "Found 45 transactions..." $\rightarrow$ "Identifying recurring subscriptions..." $\rightarrow$ "Scanning for tax deductions..."
- **Receipt Summary:** "Imported 124 transactions. I flagged 3 low-confidence items for your review."

**UX Details - Bulk Edit:**
- **Selection:** Long press on mobile / Checkbox on desktop.
- **Action:** Floating action bar appears ("Edit Category", "Ignore", "Delete").
- **Smart Suggest:** "Change all 12 'Uber' transactions to 'Transport'?"
...
interface InsightCardProps {
  ...
  scoreImpact?: number; // e.g., +5
  onDismiss: () => void;
  onAction: (actionId: string) => void;
  taxBadge?: boolean; // Shows IRS shield icon
}
```
...
interface TransactionRowProps {
  ...
  isSelected?: boolean; // For bulk edit
  onSwipeLeft?: () => void; // Quick action 1
  onSwipeRight?: () => void; // Quick action 2
  onTap: () => void; // View details
  onLongPress?: () => void; // Enter selection mode
  onCategoryChange: (category: Category) => void;
  onTeachCora: () => void; // Trigger "learning" animation
}
```

**Confidence Logic:**
- **High (>80%):** Standard display.
- **Low (<80%):** Category badge has subtle yellow outline or "Review" indicator. Tapping opens a "Quick Picker" with top 3 likely alternatives.
- **Teaching:** Changing a low-confidence category triggers a positive "Brain" animation/toast.
...
interface QuickStatCardProps {
  ...
  isSafeToSpend?: boolean; // Toggles Green/Red logic
  onClick?: () => void;
}
```

**Safe-to-Spend Logic:**
- Calculation: `Current Balance - Comfort Floor - Pending Bills`
- Visual: Green if > 0, Red if < 0.

---

**9. PrivacyShield**

Visual indicator of security status.

```typescript
interface PrivacyShieldProps {
  status: 'encrypted' | 'local' | 'syncing';
  showLabel?: boolean;
}
```

**Visuals:**
- Lock icon (Green/Teal)
- Tooltip: "Your data is end-to-end encrypted."
...
| **Pagination** | **Infinite Scroll** for Insight Feed. **Pagination** (25/50/100) for Transaction Tables to ensure performance with large datasets. |

#### Empty States

| State | Message | Action |
|-------|---------|--------|
| **First Use (Cold Start)** | "Welcome to Cora! Let's get set up." | Show **Setup Quests** in feed: "Set Comfort Floor", "Upload Statement". |
...
| **Category Change** | None (instant) | Undo toast + "Learning" feedback |
...
#### Power User Shortcuts (Categorization)

| Key | Action |
|-----|--------|
| **Up/Down** | Navigate transaction list |
| **Enter** | Open detail / Confirm category |
| **C** | Open Category Picker |
| **Del** | Delete/Ignore transaction |
| **Space** | Select row (for bulk edit) |
...
| 3 | Statement upload + processing | Drag-drop, progress, preview, privacy shield |
| 4 | Transaction list + categorization | Review queue, swipe actions, **Bulk Edit** |
...
| 8 | Mobile responsive shell | Bottom nav, core layouts, **Safe-to-Spend** |
...
| Deliverable | Status |
|-------------|--------|
| **Design System** | shadcn/ui with 9 custom components |
...
| 2025-11-28 | 1.1     | Added transparent processing, AI confidence, PT localization | UX Expert |
| 2025-11-28 | 1.2     | Added Safe-to-Spend, Bulk Actions, Score Impact | UX Expert |
| 2025-11-28 | 1.3     | Added Privacy Indicators, Shortcuts, Pagination | UX Expert |
